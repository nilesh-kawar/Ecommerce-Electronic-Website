...
const ProductCard = ({ product, id, img1, img2, isSoldOut, name, price, prevPrice, addToCart }) => {
  ...
  return (
    ...
    <div
      key={id}
      className="relative w-max m-2 pb-5 px-1 bg-white border border-gray-200 rounded-lg shadow-xl group duration-1000 overflow-hidden"
    >
      <img
      ...
      <div className="absolute top-[60%] left-1/2 -translate-x-1/2 -translate-y-1/2  flex justify-center items-center space-x-2 mb-4 opacity-0 group-hover:opacity-100 duration-300">
        {isSoldOut ? (
          ...
          <Tooltip text="Add to Cart">
            <button className="p-3 shadow-xl bg-secondary text-white text-lg inline-table hover:bg-primary rounded-full duration-200 cursor-pointer"
      ...
      <Link to={`/products/${id}`}>
        <h5 className="text-lg font-bold text-slate-600 text-center pt-3 pb-2 hover:text-primary">
          {name}
        </h5>
      </Link>
